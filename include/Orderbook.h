...
public:
    ...
    // add_order / cancel_order / match logic — stages 2-4.

    /*
    add_order
    1. Try and match it against the existing orders on the opposite side
        - involves a matching loop against price then time priorities
    2. Add the remaining as resting orders
    */
    void add(OrderId orderId, Price price, Quantity qty, Side side){
        ...
        matchOrder(order, fills_produced);

        //TODO Add a call-back to fill-processor
...
private:
    ...
    void matchAtLevel(Order& order, Level& level, std::vector<Fill>& fills_produced){
        ...
        while (order.quantity > 0 && it != level.orders.end())
        {
            ...
            fills_produced.emplace_back(it->price, matching_qty, it->id, order.id);
